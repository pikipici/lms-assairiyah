...
const FinishedLearningContainer = () => {
  const { loginToast, endErrorToast } = useCustomToast();

  const { board, addImageToBoard, removeItemFromBoard } = useFinishedLearning();

  const { removeItemFromBoard: removeNotStarted } = useNotStarted();

  const { removeItemFromBoard: removedCurrentlyLearning } =
  ...
  const onDrop = (item: DragCourseType, monitor: DropTargetMonitor) => {
    ...
    if (sourceBoard === "pending") {
      ...
      removedCurrentlyLearning(item.id);
    }
  ...
  return (
    ...
    >
      <CardHeader className="pb-3">
        <CardTitle className="text-center">Selesai dipelajari</CardTitle>
      </CardHeader>
      <div className="flex flex-col w-full pb-2">
        {board.length === 0 && (
          <p className="text-center text-muted-foreground text-sm">
            Belum ada materi yang di selesaikan.
          </p>
        )}
        {board.map((item) => {
...
export default FinishedLearningContainer;
